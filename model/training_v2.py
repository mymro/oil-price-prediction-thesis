...
n_in, m_out = 3,3
y_headers = ["brent"]
x_headers = ["vader","vader_average","watson","count","brent"]
training_split = 0.8
...
test_x = test_x.reshape(test_x.shape[0], n_in*test_x.shape[2])
y_hat = np.concatenate((test_x, y_hat), axis = 1)
y = np.concatenate((test_x, test_y), axis = 1)

y_hat = scaler.inverse_transform(y_hat.reshape((test_x.shape[0], prepared_dataset.shape[1])))[:,-m_out*len(y_headers):]
y = scaler.inverse_transform(y.reshape((test_x.shape[0], prepared_dataset.shape[1])))[:,-m_out*len(y_headers):]


pd.DataFrame(np.concatenate((y,y_hat), axis=1)).to_csv("out.csv")
