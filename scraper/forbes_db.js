...
class db_helper{
    ...
    update_tasks(){
        let task_arr = db.prepare("SELECT * FROM config WHERE done <> 1 AND site = 'forbes'").all();
        this.search_tasks = {};
    ...
    update_articles(){
        ...
        let articles_arr = db.prepare("SELECT * FROM articles WHERE fetched <> 1 AND site = 'forbes'").all();
    ...
    articleFailed(article){
        this.articles.enqueue(article);
    }
    ...
    addArticle(task, article){
            db.prepare("INSERT INTO articles(url, title, date, search_term, site) VALUES(?, ?, ?, ?, 'forbes')").run(article.url, article.title, article.date, task.id);
            this.articles.enqueue({url:article.url, title:article.title, date:article.date, search_term: task.id});
    }
}
