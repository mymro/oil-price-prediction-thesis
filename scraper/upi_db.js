const db = require('better-sqlite3')('upi.sqlite3');
...
class db_helper{
    ...
    update_tasks(){
        let task_arr = db.prepare("SELECT * FROM config WHERE done <> 1").all();
        this.search_tasks = {};
    ...
    update_articles(){
        ...
        let articles_arr = db.prepare("SELECT articles.*, config.min_date AS min_date FROM articles LEFT JOIN config ON config.id = articles.search_term WHERE articles.fetched <> 1 ORDER BY articles.id ASC").all();
    ...
    add_article(task, url){
        try {
            db.prepare("INSERT INTO articles(url, search_term) VALUES(?, ?)").run(url, task.id);
